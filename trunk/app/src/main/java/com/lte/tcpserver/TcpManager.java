    ...
    private Handler mSendHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                break;
//                case OPEN_DBM: {
////                    Log.d(TAG, "小区配置成功响应 ：" + App.get().getmList().size());
//                    //小区配置响应,开启射频
//                    boolean isConfig = true;
//                    for (StationInfo info : App.get().getMList()) {
//                        Log.d(TAG, "AppMlist..." + info.getSession() + " :" + info.isCellConfig());
//                        if (info.getType() == 4) {
...
//                                    Log.d(TAG, "发送消息..." + info.getSession().getRemoteAddress() + " :" + bytesToHexString(headData));
//                                }
//                            }
//                        }
...
//                break;
                case CLOSE_DBM: {
                    for (StationInfo info : App.get().getMList()) {
                        if (info.getType() == 4) {
                            Log.d(TAG, "AppMlist..." + info.getSession() + " :" + info.isConfigDBM());
...
//                    }
                ...
                case SHOUDONG: {
                    ...
                    System.arraycopy(headData, 0, sendData, 0, headData.length);
                    System.arraycopy(data, 0, sendData, 0 + headData.length, data.length);
                    Log.d(TAG, "DTATA :" + bytesToHexString(sendData));
                    seqNo++;
                    stationInfo.setSoft_state(mContext.get().getString(R.string.bbu_scan));
                    EventBus.getDefault().post(new MessageEvent(true));
                                ...
                                if (isContainYiDong) {
                                    if (info.getIp() != null && info.getIp().length() > 15) {
                                        if (TextUtils.equals(info.getIp().substring(15, 16), "1")
                                                ...
                                                || TextUtils.equals(info.getIp().substring(15, 16), "6")
                                                ) {
                                            postDelayed(new Runnable() {
                                                int isUpgrade = 1;
                    ...
                    for (final StationInfo info : App.get().getMList()) {
                        if (info.getType() == 4) {
                            if (info.getConnectionStatus() == StationInfo.ConnectionStatus.CONNECTED) {
                                //，关闭定位 延时两秒关闭射频
                                byte[] headData = new byte[]{0x02, 32, 0x00, 11, 36, 0x00, 0x01, 0x00, 0x01, 0x00, 0x04, (byte) ((seqNo >> 24) & 0xFF), (byte) ((seqNo >> 16) & 0xFF), (byte) ((seqNo >> 8) & 0xFF), (byte) (seqNo & 0xFF)};
                                seqNo++;
                                info.setPositionOFF(true);
                                info.getSession().write(headData);
                                if(TextUtils.equals(info.getIp().substring(13, 16), "216") ||
                                        TextUtils.equals(info.getIp().substring(13, 16), "217")){
                                    info.setIp(info.getIp(), true);//回复默认配置//FDD回复默认，TDD不恢复
                                }
                                postDelayed(new Runnable() {
                                    @Override
                                    public void run() {
                ...
                case OPEN_DBM2: {
                    for (StationInfo info : App.get().getMList()) {
                        if (info.getConnectionStatus() == StationInfo.ConnectionStatus.CONNECTED
                            ...
                            //lph 20190610 解決问题每开关一次侦码开关，就会触发一次小区更新
...
//                                        }
                        ...
                        sendEmptyMessageDelayed(POINT, intConfig * 1000L);
                    }
                    break;
                }
                case CHECK_ONLINE:
...
//                            }
                            ...
                            if (stationInfo.isClose()) {
                                Log.d("测试", stationInfo.getIp() + "--- isClose() :" + stationInfo.isIsshoudong());
                                if (msg.get(4) == 2 && msg.get(7) == 0) {
                                    //关闭DBM响应,系统状态查询
                                    byte[] headData = new byte[]{0x02, 0x0a, 0x00, 0x07, 0x01, 0x00, 0x04, (byte) ((seqNo >> 24) & 0xFF), (byte) ((seqNo >> 16) & 0xFF), (byte) ((seqNo >> 8) & 0xFF), (byte) (seqNo & 0xFF)};
                                    seqNo++;
...
//                        }
        ...
        public void sessionOpened(IoSession session) {
            Log.d(TAG, "sessionOpened " + session.getRemoteAddress());
            for (StationInfo stationInfo : App.get().getMList()) {
                if (TextUtils.equals(String.valueOf(session.getRemoteAddress()), stationInfo.getIp())) {
                    stationInfo.setConnectionStatus(StationInfo.ConnectionStatus.CONNECTED);
                    stationInfo.setSession(session);
        ...
        ThreadUtils.getThreadPoolProxy().execute(new Runnable() {
            ...
            public void run() {
                ...
                if (stationInfo.getIp().length() > 16) {
                    ...
                    if (!App.get().isClose && TextUtils.equals(stationInfo.getIp().substring(15, 16), "1")) {
                    ...
                    } else if (!App.get().isClose && TextUtils.equals(stationInfo.getIp().substring(15, 16), "2")) {
                        App.get().isClose = true;//执行完关闭其他基带板后置为true；
                        for (StationInfo info : App.get().getMList()) {
                    ...
                    } else if (!App.get().isClose && TextUtils.equals(stationInfo.getIp().substring(15, 16), "3")) {
                        App.get().isClose = true;//执行完关闭其他基带板后置为true；
                        for (StationInfo info : App.get().getMList()) {
                    ...
                    } else if (!App.get().isClose && TextUtils.equals(stationInfo.getIp().substring(15, 16), "6")) {
                        App.get().isClose = true;//执行完关闭其他基带板后置为true；
                        for (StationInfo info : App.get().getMList()) {
                    ...
                    } else if (!App.get().isClose && TextUtils.equals(stationInfo.getIp().substring(15, 16), "7")) {
                        App.get().isClose = true;//执行完关闭其他基带板后置为true；
                        for (StationInfo info : App.get().getMList()) {
...
//                        }
                        Log.d("messageSent", "---" + stationInfo.getmList().size());
//                    }
//                    if (stationInfo.isShoudongSend()) {
//                        stationInfo.getmList().add(scanResult);
//                        if (scanResult.getRSSI() != 65535) {
